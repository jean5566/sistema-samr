    ...
    <div className="min-h-screen text-slate-900 font-sans overflow-y-auto flex flex-col">
      ...
      <div className="flex-1 px-4 sm:px-8 pb-12 w-full max-w-7xl mx-auto">
        ...
        <div className="bg-gradient-to-r from-blue-700 to-blue-500 rounded-[2rem] px-5 py-5 sm:px-8 sm:py-8 text-white flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-6 shadow-lg shadow-blue-500/20 mb-4 sm:mb-6">
          <div>
            <p className="text-blue-200 text-xs sm:text-sm font-bold uppercase tracking-wide mb-1">Bienvenido</p>
            <h2 className="text-xl sm:text-3xl font-black tracking-tight">{user?.name}</h2>
            <p className="text-blue-100 text-xs sm:text-sm mt-1 sm:mt-2 font-medium">{user?.email}</p>
          </div>
          {periodo && (
            <div className="text-left sm:text-right">
              <p className="text-blue-200 text-xs sm:text-sm font-bold uppercase tracking-wide mb-1">Período actual</p>
              <p className="text-xl sm:text-3xl font-black tracking-tight">{periodo}</p>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-5">
          <button onClick={() => navigate('/dashboard/estudiante/docentes')}
            className="bg-white border border-slate-100/60 rounded-[2rem] p-4 sm:p-6 flex items-center gap-3 sm:gap-5 hover:bg-slate-50 hover:border-slate-200 transition-all duration-300 shadow-sm hover:shadow-md group text-left">
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-blue-50 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform duration-300 shadow-sm">
            ...
            <div>
              <p className="text-sm sm:text-base font-bold text-slate-900 group-hover:text-blue-600 transition-colors">Mis docentes</p>
              <p className="text-xs sm:text-sm font-medium text-slate-500 mt-0.5 sm:mt-1">Ver información de contacto y solicitar citas</p>
            </div>
          </button>
          <button onClick={() => navigate('/dashboard/estudiante/documentos')}
            className="bg-white border border-slate-100/60 rounded-[2rem] p-4 sm:p-6 flex items-center gap-3 sm:gap-5 hover:bg-slate-50 hover:border-slate-200 transition-all duration-300 shadow-sm hover:shadow-md group text-left">
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-violet-50 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform duration-300 shadow-sm">
              <svg className="w-5 h-5 sm:w-6 sm:h-6 text-violet-600" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
              </svg>
            </div>
            <div>
              <p className="text-sm sm:text-base font-bold text-slate-900 group-hover:text-violet-600 transition-colors">Documentos</p>
              <p className="text-xs sm:text-sm font-medium text-slate-500 mt-0.5 sm:mt-1">Reglamentos, sílabos y formularios</p>
            </div>
          </button>
          <button onClick={() => navigate('/dashboard/estudiante/mis-archivos')}
            className="bg-white border border-slate-100/60 rounded-[2rem] p-4 sm:p-6 flex items-center gap-3 sm:gap-5 hover:bg-slate-50 hover:border-slate-200 transition-all duration-300 shadow-sm hover:shadow-md group text-left">
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-emerald-50 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform duration-300 shadow-sm">
            ...
            <div>
              <p className="text-sm sm:text-base font-bold text-slate-900 group-hover:text-emerald-600 transition-colors">Mis archivos</p>
              <p className="text-xs sm:text-sm font-medium text-slate-500 mt-0.5 sm:mt-1">Tu almacenamiento privado, solo visible para ti</p>
            </div>
          </button>
        </div>
